...
│   ├── mac-M/                        # Modules for mac M‑series devices
│   │   ├── audio_recording_mac.py    # Audio Recording Module for mac M‑series
│   │   ├── birdnet_analyzer_mac.py   # BirdNET Analyzer Module for mac M‑series
│   │   └── birdnet_image_demo_mac.py # Bird Image Demo Module for mac M‑series
│   ├── audio_recording.py            # Audio Recording Module for Raspberry Pi
│   ├── birdnet_analyzer.py           # BirdNET Analyzer Module for Raspberry Pi
│   ├── birdnet_image_demo.py         # Bird Image Demo Module for Raspberry Pi
│   ├── dashboard.py                  # Dashboard backend module
│   └── ...                           # Other module-specific files
├── config/                           # User-modifiable configuration files
│   └── module_config.yaml            # Consolidated YAML config for global parameters and module definitions
├── module_dockerfiles/               # Dockerfiles for building containers for each module
│   ├── mac-M/                        # Dockerfiles for mac M‑series modules
│   │   ├── Dockerfile.audio_recording_mac
│   │   ├── Dockerfile.birdnet_analyzer_mac
│   │   └── Dockerfile.birdnet_image_demo_mac
│   ├── Dockerfile.audio_recording     # For Raspberry Pi
│   ├── Dockerfile.birdnet_analyzer      # For Raspberry Pi
│   └── Dockerfile.birdnet_image_demo    # For Raspberry Pi
├── dashboard_frontend/               # Bundled React/JS/CSS for the dashboard frontend
│   ├── index.html                    # Main HTML file for the dashboard
│   ├── main.js                       # React/JS code
│   └── styles.css                    # Custom CSS (Material-UI via CDN)
├── local_data/                       # Shared data folder (recordings, ledger CSV, images, etc.)
├── log/                              # Logs and snapshots of Docker Compose commands
│   ├── all-docker-compose-mac.yml    # Compose file for all non-dashboard modules on mac M‑series
│   ├── all-docker-compose.yml        # Compose file for all non-dashboard modules on Raspberry Pi
│   └── current-docker-compose.yml    # Recreated compose file generated by the dashboard based on user choices
├── docker-compose.yml                # Docker Compose file for spinning up the Dashboard container
